...
private:
	...
	bool Add(Node* &node, const int &dataValue);
	bool Remove(Node* &node, const int &dataValue);
	void DeleteNode(Node* &node);
	int RemoveSmallest(Node* &parent);
	int CountLevels(Node* root);
	void DisplayLevels(Node* root, std::ostream& outputStream);
